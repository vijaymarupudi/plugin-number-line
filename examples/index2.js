...
(async () =>
{
    ...
    function onDrag(e)
    {
        ...
        // handle.x = slider.toLocal(e.global).x;
        if (type == "universal") {
            handle.x = Math.max(halfHandleWidth, slider.toLocal(e.global).x);
            ...
            handle.x = Math.max(halfHandleWidth, Math.max(slider.toLocal(e.global).x, sliderWidth - halfHandleWidth));
        }

        // handle.x2 = (universal) ?  sliderWidth - halfHandleWidth :  slider.toLocal(e.global).x;
        // handle.x = Math.max(halfHandleWidth, Math.min(slider.toLocal(e.global).x, handle.x2));
